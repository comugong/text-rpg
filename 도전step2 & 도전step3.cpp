...
protected:
	...
	int maxExp;
public:
	Player(const string& name, string job, int hp, int mp, int ap, int dp) :name(name), job(job), hp(hp), mp(mp), ap(ap), dp(dp), level(1), exp(0), maxExp(100) {}
...
public:
	Monster(const string& name, int hp, int ap, int dp, const string& dropItemname, int dropItemPrice) : name(name), hp(hp), ap(ap), dp(dp), dropItem{dropItemname, dropItemPrice}, expReward(0) {}
