...
void fight(Player& player, Monster*& monster, vector<Item>& inventory);
void gameMenu(Player& player, Monster*& monster, vector<Item>& inventory, vector<Potion>& potion);
...
void setPotion(int count, int* hpPotion, int* mp_Potion);
...
int main(void) {                                                // 캐릭터 생성 화면
	...
	gameMenu(*player, monster, inventory, potion);
	...
	while (true) {
		...
		switch (menuNumber) {	
			case 0:
				...
				return;
			case 1:
				fight(player, monster, inventory);
				break;
			case 2:
...
public:
	Monster(const string& name, int hp, int ap, int dp, const string& dropItemname, int dropItemPrice) : name(name), hp(hp), ap(ap), dp(dp), dropItem{dropItemname, dropItemPrice} {}
